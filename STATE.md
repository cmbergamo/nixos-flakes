...
| server/base.nix | Módulo base headless do servidor: tuning de memória (zram zstd 50%, earlyoom), sysctl de rede, GC do nix, systemd-networkd com DHCP automático em todas as interfaces cabeadas e wireless (`en* eth* wl* wlan*`), resolved com fallback DNS 1.1.1.1/8.8.8.8, OpenSSH porta 22, fail2ban, Podman, terminfo universal e pacotes essenciais | networkd exige .network configurado para DHCP em todas as interfaces físicas (`en* eth* wl* wlan*`), com `ActivationPolicy = "always-up"` e serviço oneshot `bring-network-interfaces-up` em `network-pre.target` para evitar que placas ethernet (eno1/eno2) inicializem desligadas (DOWN); `enableAllTerminfo = true` e `wezterm.terminfo` evitam quebras de linha e rolagem espúria no SSH; wait-online.anyInterface evita hang de boot em portas desconectadas; useDHCP=false evita avisos do NixOS |
...
| nushell.nix | módulo universal Nushell: publica /etc/nushell/config.nu e /etc/skel/.config/nushell/config.nu, cria serviço user nushell-config-setup | config.nu desativa banner (`show_banner = false`), desativa sequências OSC 133 (`shell_integration.osc133 = false`) para evitar corrupção de terminal via SSH, e garante fallback de TERM para xterm-256color |
...
- **NÃO usar `ActivationPolicy` em `systemd.network.links` ou `systemd.network.networks.<name>.linkConfig`**: no esquema do NixOS, o bloco de link do systemd tem tipos estritos e gera `TypeError` ou warnings de campos extras (`trace: Systemd Link has extra fields`). A ativação de placas no boot deve ser feita via serviço oneshot em `network-pre.target` (`bring-network-interfaces-up`).
